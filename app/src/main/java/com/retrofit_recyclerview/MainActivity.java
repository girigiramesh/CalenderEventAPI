...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.btn:
                trackervigilApi.getCalender(bearer, domainName).enqueue(new Callback<String>() {
                    @Override
...
//                                    calender.setBackgroundColor(getResources().getColor(R.color.colorAccent));
                ...
                break;
            case R.id.btn1:
                trackervigilApi.login(contentType, grantType, email, pass, domainName).enqueue(new Callback<String>() {
                    @Override
                    public void onResponse(Call<String> call, Response<String> response) {
                        if (response.isSuccessful()) {
                            Log.d(TAG, "onResponse: " + response.body());
